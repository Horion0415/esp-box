...
static esp_lcd_panel_handle_t s_lcd_panel[NUM_EYES];
static esp_lcd_panel_io_handle_t s_lcd_io[NUM_EYES];

/**
 * @brief Initialize LCD panel
 * 
 * @param eye_index Index of the eye to initialize
...
static esp_err_t init_lcd_panel(int eye_index) 
{
    ...
    esp_err_t ret = bsp_display_new(&bsp_disp_cfg, &s_lcd_panel[eye_index], &s_lcd_io[eye_index]);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Failed to initialize LCD panel #%d", eye_index);
        return ret;
    }
    
    ret = esp_lcd_panel_disp_on_off(s_lcd_panel[eye_index], true);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Failed to turn on LCD panel #%d", eye_index);
        return ret;
    }
...
void app_main(void) 
{
    ...
    for (int i = 0; i < NUM_EYES; i++) {
        if (init_lcd_panel(i) != ESP_OK) {
            ESP_LOGE(TAG, "Failed to initialize LCD panel #%d", i);
            return;
        }
    }
    ...
    if (eye_animation_init(s_lcd_panel, eyeInfo, NUM_EYES) != ESP_OK) {
        ESP_LOGE(TAG, "Failed to initialize eye animation");
        return;
    }
